...
class TextHandler:
    ...
    async def _main_menu(self):
        ...
        kb = keyboards.get_main_menu(self.lang)
    ...
    async def _setlang(self):
        """
        When user picks a language, this method saves it to db and redicts user to main
        menu
        """
        # db_operations.setlang(self.user_id, self.lang)
        # todo lang db
        await self._main_menu()
    ...
    async def _request_location(self):
        """
        When user wants to change location, bot sends him a keyboard with instructions
        """
        await self.bot.send_chat_action(self.user_id, ChatActions.TYPING)
        kb = keyboards.get_geobutton(self.lang)
