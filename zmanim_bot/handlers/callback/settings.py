...
from zmanim_bot.keyboards.inline import (
...
from zmanim_bot.helpers import CallbackPrefixes, CALL_ANSWER_OK
...
async def handle_cl_call(call: CallbackQuery):
    ...
    asyncio.create_task(storage_api.get_or_set_cl(cl))
...
async def handle_havdala_call(call: CallbackQuery):
    ...
    await bot.edit_message_reply_markup(call.from_user.id, call.message.message_id, reply_markup=kb)
    asyncio.create_task(storage_api.get_or_set_havdala(havdala))
...
async def handle_omer_call(call: CallbackQuery):
    ...
    if omer_flag:
        zmanim = await zmanim_api.get_zmanim(
            await storage_api.get_or_set_location(),
            zmanim_api.Zmanim().dict()
        )
    else:
        zmanim = None
    asyncio.create_task(storage_api.get_or_set_omer_flag(omer_flag, zmanim))
...
async def handle_zmanim_call(call: CallbackQuery):
    ...
    await bot.edit_message_reply_markup(call.from_user.id, call.message.message_id, reply_markup=kb)
    asyncio.create_task(storage_api.get_or_set_zmanim(current_zmanim))
